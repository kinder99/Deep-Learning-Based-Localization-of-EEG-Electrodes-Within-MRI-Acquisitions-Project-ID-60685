...
### VALIDATION
###
### Compare the ground truth segmentation with the predicted segmentation for the first model.
###
...
gTruth_image = sitk.ReadImage('c:/Users/User/Desktop/TED/Results_GroundTruth/UTE/S_1/60K/gt.nii')
#predict_image = sitk.ReadImage('c:/Users/User/Desktop/TED/Results_Predictions/Brut/Pred_UTE_V1/Hemisfer_125.nii.gz')
predict_image = sitk.ReadImage('c:/Users/User/Desktop/TED/Results_Predictions/Post-Processed/PostProcessed_UTE_V1/Hemisfer_101_postprocessed.nii.gz')
...
for i in range(0, len(predict_centers)) :
    ...
    print('Composante {0} : {1} corresponding to {2}, position error = {3} mm'.format(i+1, predict_centers[i], corresponding_gTruth_centers[i], position_error[i]))
...
for index in range(0, len(position_error)) :
    ...
    if (position_error[index] > 10) :
        ...
        print('Outlier : {0}'.format(predict_centers[index]))
